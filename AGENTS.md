...
**最新 commit**：`4ce2f1b fix: build type errors blocking production build`

**已完成**：
- 2 款游戏 × 8 个工具 = 完整功能 UI（OT2 已补攻略工具，达到 2 类最低要求）
...
- **🎉 线上部署**：`https://gametoolx.top`（HTTPS + TrustAsia SSL + aaPanel Node 项目跟踪 + pm2 自动重启）
- 完整部署 skill 沉淀到 `~/.mavis/agents/mavis/skills/aapanel-node-deploy/`

**未做（按优先级）**：
1. **AI Q&A**（`/api/qa`）：searchChunks → LLM prompt → 流式回答。
2. **SSL 续期**：TrustAsia 3 个月，2026-09-05 到期，9 月初在 aaPanel 续期。
3. **更多 crawler**：NGA（zh）、3DM（zh）、GameWith（ja）、Steam News。
4. **向量搜索**：`sqlite-vss` 或迁 `pgvector`。
5. **i18n 收尾**：`SystemCheckerClient.tsx` 残留英文。
6. **加 zh-TW 繁体中文**（台湾市场）。
7. **第三款游戏**：验证多游戏架构可扩展性。
...
## 生产部署（参考，详见 `aapanel-node-deploy` skill）

### 线上环境
- **域名**：`https://gametoolx.top`（DNS 已解析到 server）
- **服务器**：腾讯云 ECS，IP `124.156.229:149`，OpenCloudOS 9.4，3.5GB RAM / 60GB 磁盘
- **控制面板**：宝塔面板腾讯云专享版，端口 8888
- **SSL**：TrustAsia DV TLS RSA CA 2024，**有效期 3 个月**（2026-06-07 → 2026-09-05）
- **Node.js**：v24.16.0，路径 `/www/server/nodejs/v24.16.0/`
- **进程管理**：pm2，进程名 `gametoolx`，运行用户 `www`
- **反向代理**：nginx（aaPanel 自动配置 + 手工修 well-known typo + 加 X-Forwarded-Proto）

### SSH 访问
私钥在 `D:\Idea Project\dream\.ssh-tmp\gametoolx-root-2026-06-07`（ACL 只给 zy187 读）。
权限 600 才有 `OpenSSH` 能用。
...
```powershell
# 进程
ssh -i <key> root@124.156.229.149 "pm2 list"     # status=online, pid 稳定

# HTTPS 主页
curl -I https://gametoolx.top                     # 200 或 307
curl -I https://gametoolx.top/zh/games/ot2        # 200

# HTTP→HTTPS 重定向
curl -I http://gametoolx.top                      # 301 + Location: https://...

# SSL 证书有效期
echo | openssl s_client -servername gametoolx.top -connect 124.156.229.149:443 2>/dev/null | openssl x509 -noout -dates
```

### 常见部署坑
- **`next build` 报 TS 类型错**：dev 模式不严格，build 严格。本地先 `npx tsc --noEmit` 和 `npx next build` 都过再 push。
- **`next build` OOM**：3.5GB RAM 装 Next.js build 紧张。已加 2GB swap。
- **curl 124.156.229.149:3000 超时**：firewall/安全组不开 3000，必须走 nginx 80/443 反代。
- **aaPanel UI 看不到项目**：直接写 `site.db.sites` 后必须 `bt restart` 让 panel 重读 SQLite WAL。
- **aaPanel 显示项目但状态"未运行"**：inner JSON `project_type` 必须是 `"pm2"`（不是 `"nodejs"`），且 `pm2_name` 必须匹配 `pm2 start --name X` 的 X。
- **DB 已 scp 上传但 search 仍空**：pm2 进程内存里 DB connection 缓存了旧 inode，必须 `pm2 restart`。

### 完整部署流程
参考 `C:\Users\zy187\.mavis\agents\mavis\skills\aapanel-node-deploy\SKILL.md`（含 nginx 模板、site.db 注册模板、错误速查表）。
...
- **不要**本地 build 没跑过就 push 到 `master`。服务器 `next build` 才严格检查类型（dev 模式不查），本地不验就推到 master → 服务器部署挂。
- **不要**直接改 `data/gametoolx.db*` 然后 push（DB 是 gitignored，别误加）。`*.db-wal` 4MB 大小容易误中招。
- **不要**用 `next build` 不加 `NODE_OPTIONS=--max-old-space-size=2560`（服务器 3.5GB RAM 不够）。
- **不要**通过 `124.156.229.149:3000` 直连测试（防火墙挡）。用 `https://gametoolx.top` 测，nginx 80/443 走反代。
- **不要**在 aaPanel UI 上"删项目"——会触发 `bt restart` 重读 DB，丢失手动注册的项目。直接 SQL 删/改。
