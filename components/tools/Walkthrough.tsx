  ...
  return (
    <div className="lg:grid lg:grid-cols-[260px_1fr] lg:gap-8">
      ...
      <aside className="hidden lg:block">
        <nav className="sticky top-4 rounded-2xl border border-white/10 bg-white/5 p-4 max-h-[calc(100vh-2rem)] overflow-y-auto">
      ...
      <details className="lg:hidden rounded-2xl border border-white/10 bg-white/5 p-4">
        <summary className="cursor-pointer text-sm font-semibold text-brand-300 uppercase tracking-wide">
          📑 {lang === "zh" ? "目录" : "Contents"}
        </summary>
        <ul className="mt-3 space-y-1 text-sm">
          {sections.map((s) => (
            <li key={s.id}>
              <a
                href={`#section-${s.id}`}
                className="block rounded px-2 py-1.5 text-gray-300 hover:bg-white/5 hover:text-white"
              >
                {t(s.title)}
              </a>
            </li>
